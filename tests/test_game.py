...
def test_refreshEnemies(mock_initEnemiesGroup, game):
    mock_initEnemiesGroup.return_value = [
        Mock(),
        Mock(),
        Mock(),
        Mock(),
        Mock(),
    ]  # Creo el mock de un grupo de enemigos
...
def test_catchControllerEvents(mock_quit, mock_get_pressed, game):
    mock_get_pressed.return_value = {
        ...
        K_RIGHT: 0,
        K_z: 0,
    }  # verifico que solo se haya presionado una tecla
...
def test_runGame(
    ...
    mock_catchEvents.side_effect = [
        None,
        Exception("Stop loop"),
    ]  # side_effect simula un comportamiento de la funcion catchEvents
    try:
        game.runGame(
            mock_screen, mock_clock, mock_playerGroup, mock_enemiesGroup, mock_road
        )
    except Exception:
        pass
    ...
    mock_catchControllerEvents.assert_called_once_with(
        mock_road, mock_playerGroup.sprites()[0], mock_enemiesGroup
    )
    mock_refreshEnemies.assert_called_once()
    mock_catchCollisions.assert_called_once_with(
        mock_playerGroup.sprites()[0], mock_enemiesGroup
    )
